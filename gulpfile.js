...
var eurolife = require("./scripts/eurolife.js");
...
helix.header("The Eurolife source code, tools and processes.", 
    "A Wedia Project");
...
gulp.task("Publish-Assemblies",
    function() {
        var root = "./src";
        var binFiles = root + "/**/code/**/bin/Wedia.{Feature,Foundation,Eurolife}.*.{dll,pdb}";
...
gulp.task("Publish-All-Projects", gulp.series(
    "Build-Solution",
...
gulp.task("References-Local", gulp.series(function (callback) {
...
gulp.task("Auto-Publish-Assemblies",
    function() {
        ...
        var files = "/**/Wedia.{Feature,Foundation,Eurolife}.*.{dll,pdb}";;
        var destination = config.websiteRoot + "/bin/";
        return gulp.src(roots, { base: root }).pipe(
            foreach(function(stream, rootFolder) {
                gulp.watch(rootFolder.path + files,
...
gulp.task("Package-Create-Package-Xml",
    function(callback) {
        return gulp.src(config.packageXmlBasePath, { allowEmpty: true }).pipe(rename(config.solutionName + ".xml")).pipe(gulp.dest(config.packagePath));
    });
...
gulp.task("Package-Prepare-Package-Files",
    function(callback) {
        var excludeList = [
            ...
            "!" + config.websiteRoot + "\\bin\\Wedia.{Feature,Foundation,Eurolife,Common}*dll"
        ];
        console.log(excludeList);

        return gulp.src(excludeList, { read: false, allowEmpty: true }).pipe(rimraf({ force: true }));
    });
